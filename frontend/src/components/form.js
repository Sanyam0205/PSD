...
14. Delivery Schedule: urgent basis.`);
  ...
  useEffect(() => {
    const fetchVendor = async () => {
      try {
        if (vendorCode) {
          const response = await axios.get(`http://ec2-13-201-68-74.ap-south-1.compute.amazonaws.com:5000/api/vendors/${vendorCode}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post('http://ec2-13-201-68-74.ap-south-1.compute.amazonaws.com:5000/api/project-orders', projectOrder);
      // Generate PDF
      const doc = generatePDF();
