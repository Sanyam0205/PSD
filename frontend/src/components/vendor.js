...
const VendorManagement = () => {
  ...
  const fetchVendors = async () => {
    try {
      const response = await axios.get('http://ec2-13-201-175-136.ap-south-1.compute.amazonaws.com:5000/api/vendors');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (formData.vendorCode) {
        ...
        if (existingVendor) {
          await axios.put(`http://ec2-13-201-175-136.ap-south-1.compute.amazonaws.com:5000/api/vendors/${formData.vendorCode}`, formData);
        } else {
          await axios.post('http://ec2-13-201-175-136.ap-south-1.compute.amazonaws.com:5000/api/vendors', formData);
        }
      } else {
  ...
  const handleDelete = async (code) => {
    ...
    if (confirmDelete) {
      try {
        await axios.delete(`http://ec2-13-201-175-136.ap-south-1.compute.amazonaws.com:5000/api/vendors/${code}`);
        fetchVendors();
      } catch (error) {
