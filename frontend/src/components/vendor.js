...
const VendorManagement = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (formData.vendorCode) {
        ...
        if (existingVendor) {
          await axios.put(`http://127.0.0.1:5000/api/vendors/${formData.vendorCode}`, formData);
        } else {
          await axios.post('http://127.0.0.1:5000/api/vendors', formData);
        }
      } else {
  ...
  const handleDelete = async (code) => {
    ...
    if (confirmDelete) {
      try {
        await axios.delete(`http://127.0.0.1:5000/api/vendors/${code}`);
        fetchVendors();
      } catch (error) {
